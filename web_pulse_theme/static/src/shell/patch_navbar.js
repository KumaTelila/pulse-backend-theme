...
patch(NavBar.prototype, {
    ...
    get pulseRouteSegments() {
        ...
        const name =
            ctrl?.displayName ||
            (ctrl?.action?.tag === "pulse_dashboard" ? "Overview" : "") ||
            this.currentApp?.name ||
            "";
        if (!name) {
            return [];
        }
        return [{ jsId: "pulse-fallback", name, isLast: true, onSelected: null }];
    },

    onPulseHistoryBack(ev) {
        ev.preventDefault();
        browser.history.back();
    },

    onPulseHistoryForward(ev) {
        ev.preventDefault();
        browser.history.forward();
    },

    onPulseCrumbClick(seg, ev) {
        ev.preventDefault();
        seg.onSelected?.();
    },
    ...
    toggleMobileSidebar(ev) {
        ev.preventDefault();
        document.body.classList.toggle('o_pulse_sidebar_mobile_open');
    },
