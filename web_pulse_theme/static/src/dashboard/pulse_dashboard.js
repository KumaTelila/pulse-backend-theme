...
export class PulseDashboard extends Component {
    static template = "web_pulse_theme.PulseDashboard";
    ...
    async load() {
        ...
        try {
            const payload = await this.orm.call("pulse.dashboard", "get_data", [], {
                tab: this.state.tab,
    ...
    getPrimaryColor() {
        const v = getComputedStyle(document.documentElement)
            .getPropertyValue("--pulse-primary")
            .trim();
        return v || "#5b4fec";
    }
    ...
    rowStatusClass(state) {
        const m = {
            draft: "o_pulse_dash_status_pending",
            sent: "o_pulse_dash_status_inprogress",
            sale: "o_pulse_dash_status_completed",
            done: "o_pulse_dash_status_completed",
            cancel: "o_pulse_dash_status_cancelled",
        };
        return m[state] || "o_pulse_dash_status_inprogress";
    }
    ...
    async onExportClick() {
        ...
        try {
            const payload = await this.orm.call("pulse.dashboard", "export_sales", [], {
                tab: this.state.tab,
                period_days: this.state.periodDays,
            });
            ...
            link.href = url;
            link.download = payload.filename || "pulse-sales.csv";
...
registry.category("actions").add("pulse_dashboard", PulseDashboard);
